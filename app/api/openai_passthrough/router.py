...
async def responses_create(
    ...
    # With web search off there is nothing to serve the tool with, and mantle
    # rejects the variant, so drop it and answer without search rather than
    # failing a request whose other tools are all serviceable.
    if is_responses_web_search_request(body) and not settings.enable_web_search:
